...
毎朝チェックしたい技術情報を1か所にまとめてトリアージする。

- **今すぐ読む** — 話題のトレンド・速報記事
- **後で読む** — じっくり読みたい技術ブログ・書籍情報
- **イベント確認** — 近日開催の技術勉強会・カンファレンス
...
- **記事／イベント／書籍タブ**: 用途別に切り替えて閲覧、メディアごとに表示3件固定
- **タップで展開**: 記事行をタップすると要約をインライン表示（ホバー式プレビューは廃止）
- **AI要約**: Gemini が記事本文から「主題＋3行要約」を生成し、Web版・RSS・Markdownに反映（APIキー設定時のみ。未設定ならRSS抜粋を表示）
- **ダークモード対応**: テーマ切り替えボタンでライト/ダークを切り替え可能
- **アーカイブ**: 年月タブ＋カレンダーの1ページで過去のニュースを日付から素早く探せる
...
「件数」はフィードからの取得件数です。サイト（記事タブ）ではメディアごとに新着3件のみを表示します。
